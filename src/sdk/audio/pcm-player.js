class PCMPlayer {
  constructor(context, gain, option) {
  ...
  flush() {
    ...
    this.bufferSource_list.push(bufferSource);
    bufferSource.onended = (event) => {
      this.bufferSource_list.shift();
  ...
  stop() {
    // console.log('player stop', this.startTime);
    for (let source of this.bufferSource_list) {
      source.stop();
    }
    this.bufferSource_list = [];
    this.startTime = this.audioCtx.currentTime;
