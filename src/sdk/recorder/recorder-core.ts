...
let DestroyList: { [key: string]: Function } = {};
let ID = 0;
...
let Connect = (streamStore: any, isUserMedia?: boolean | Number) => {
  // console.log(streamStore, Recorder);
  console.log(streamStore.BufferSize, isUserMedia);
  let bufferSize = streamStore.BufferSize || Recorder.BufferSize;

  let ctx: any = Recorder.Ctx,
    stream = streamStore.Stream;
  let mediaConn = (node: AudioNode) => {
    let media = (stream._m = ctx.createMediaStreamSource(stream));
    let ctxDest = ctx.destination;
    let cmsdTxt = "createMediaStreamDestination";
    if (ctx[cmsdTxt]) {
      ctxDest = ctx[cmsdTxt]();
    }
    media.connect(node);
  ...
  //浏览器回传的音频数据处理
  let onReceive = (float32Arr: Float32Array) => {
    for (let k0 in calls) {
      if (k0) {
        //避免ts检查未使用
      }
  ...
  let oldIsBest =
    ...
    "。";
  let oldScript = () => {
    console.log("===================连接方式三=====oldScript====================");
    isWorklet = stream.isWorklet = false;
    _Disconn_n(stream);
    let log_params = "Connect采用老的" + scriptProcessor + "，";
    log_params += Recorder.ConnectEnableWorklet ? "但已" : "可";
    log_params += "设置" + RecTxt + ".ConnectEnableWorklet=true尝试启用" + audioWorklet + webMTips + oldIsBest;
    console.log(log_params, 3);
  ...
  let connWorklet = () => {
    ...
    console.log(isWorklet, ctx[audioWorklet], AwNode, Recorder.ConnectEnableWorklet);
    ...
    let clazzUrl = () => {
      ...
      let clazz = "class " + RecProc + " extends AudioWorkletProcessor{ constructor ";
      clazz += xf(function (this: any, option) {
        // DEL_super(option);
      ...
      clazz += "process ";
      clazz += xf(function (this: any, input: any) {
        //需要等到ctx激活后才会有回调
    ...
    let nodeAlive = (stream._na = () => {
      ...
      if (badInt !== "") {
        ...
        badInt = setTimeout(() => {
          badInt = 0;
          if (awNext()) {
            console.log(audioWorklet + "未返回任何音频，恢复使用" + scriptProcessor, 3);
    ...
    let createNode = () => {
      ...
      let log_params = "Connect采用" + audioWorklet + "，设置";
      log_params += RecTxt + ".ConnectEnableWorklet=false可恢复老式" + scriptProcessor + webMTips + oldIsBest;
      console.log(log_params, 3);
    };
    ...
    ctx.resume()[calls && "finally"](() => {
      ...
      let url = clazzUrl();
      let res = ctx[audioWorklet].addModule(url);
      res
        .then(() => {
          if (!awNext()) return;
          ctx[RecProc] = 1;
  ...
  if (typeof connWorklet) {
    //防止ts报错
  }

  //===================连接方式一=========================
  let connWebM = () => {
    console.log("===================连接方式===connWebM======================");
    //尝试开启MediaRecorder录制webm+pcm处理
    let MR = window["MediaRecorder"];
    isWebM = stream.isWebM = Recorder.ConnectEnableWebM;

    let supportMR = MR && "ondataavailable" in MR.prototype && MR.isTypeSupported("audio/webm; codecs=pcm");
    webMTips = supportMR ? "" : "（此浏览器不支持MediaRecorder.WebM.PCM）";
    console.log(isUserMedia, isWebM, MR.prototype, MR.isTypeSupported("audio/webm; codecs=pcm"), supportMR);
    ...
    stream._ra = () => {
      ...
      if (badInt !== "") {
        ...
        badInt = setTimeout(() => {
          ...
          if (mrNext()) {
            console.log("MediaRecorder 未返回任何音频，降级使用" + audioWorklet, 3);
            // connWorklet();
          }
        }, 500);
      }
    ...
    let mrSet = { mimeType: "audio/webm; codecs=pcm" };
    //考虑检测浏览器是否支持某个 mimeType。可以使用 MediaRecorder.isTypeSupported() 方法来进行检测
    if (!MediaRecorder.isTypeSupported(mrSet.mimeType)) {
      mrSet = { mimeType: "audio/webm; codecs=opus" };
    }
    if (!MediaRecorder.isTypeSupported(mrSet.mimeType)) {
      mrSet = { mimeType: "audio/webm" };
    }
    if (!MediaRecorder.isTypeSupported(mrSet.mimeType)) {
      mrSet = { mimeType: "audio/wav" };
    }
    if (!MediaRecorder.isTypeSupported(mrSet.mimeType)) {
      mrSet = { mimeType: "audio/mp3" };
    }

    if (Recorder.ConnectWebMOptions) {
      mrSet = Object.assign(mrSet, Recorder.ConnectWebMOptions);
    }
    ...
    mr["ondataavailable"] = function (e) {
      ...
      reader.onloadend = function () {
        if (mrNext()) {
          let f32arr: any = WebM_Extract(new Uint8Array(reader.result as ArrayBuffer), webmData);
          if (!f32arr) return;
          if (f32arr == -1) {
            //无法提取，立即降级
            connWorklet();
          ...
          console.log("MediaRecorder多余回调", 3);
        }
      };
      reader.readAsArrayBuffer(e.data);
    };
    mr.start(~~(bufferSize / 48)); //按48k时的回调间隔
    let log_params = "Connect采用MediaRecorder.WebM.PCM，设置";
    log_params += RecTxt + ".ConnectEnableWebM=false可恢复使用" + audioWorklet + "或老式" + scriptProcessor;
    console.log(log_params);
  };

  connWebM();
};
let ConnAlive = (stream: any) => {
...
let Disconnect = (streamStore: any = Recorder) => {
...
let WebM_Extract = (inBytes: Uint8Array, scope: any) => {
  // console.log("=======从WebM字节流中提取pcm数据，提取成功返回Float32Array，失败返回null||-1=====");
  // console.log(inBytes);
  // console.log(scope);
  if (!scope.pos) {
    scope.pos = [0];
    ...
    while (position[0] < bytes.length) {
      ...
      if (BytesEq(eid0, [0x16, 0x54, 0xae, 0x6b])) {
        while (pos0[0] < bytes0.length) {
          ...
          let bytes1: any = readMatroskaBlock(bytes0, pos0);
          let pos1 = [0],
            track: any = { channels: 0, sampleRate: 0 };
          if (BytesEq(eid1, [0xae])) {
            //TrackEntry
            while (pos1[0] < bytes1.length) {
              let eid2 = readMatroskaVInt(bytes1, pos1);
              let bytes2: any = readMatroskaBlock(bytes1, pos1);
              let pos2 = [0];
              if (BytesEq(eid2, [0xd7])) {
                ...
                while (pos2[0] < bytes2.length) {
                  ...
                  let bytes3: any = readMatroskaBlock(bytes2, pos2);
                  //采样率、位数、声道数
  ...
  while (position[0] < bytes.length) {
    ...
    if (BytesEq(eid1, [0xa3])) {
      ...
      if (!track) {
        ...
        let u8arr: any = new Uint8Array(bytes1.length - 4);
  ...
  if (dataLen) {
    ...
    if (track0.channels > 1) {
      //多声道，提取一个声道
      let arr2: any = [];
      for (let i = 0; i < arr.length; i += track0.channels) {
        arr2.push(arr[i]);
      }
...
//两个字节数组内容是否相同
let BytesEq = (bytes1: Uint8Array | any, bytes2: number[]) => {
...
//字节数组BE转成int数字
let BytesInt = (bytes: Uint8Array | any) => {
...
//读取一个可变长数值字节数组
let readMatroskaVInt = (arr: Uint8Array | any, pos: number[], trim?: boolean | Number) => {
  let i = pos[0];
...
//读取一个自带长度的内容字节数组
let readMatroskaBlock = (arr: Uint8Array | any, pos: number[]) => {
...
class Recorder {
  ...
  static ConnectWebMOptions: any;
  static Stream: MediaStream | null = null;
  ...
  */
  ...
  static Sync: { O: number; C: number } = { /*open*/ O: 9, /*close*/ C: 9 };
  static Scope: any;
  ...
  set: any;
  id: number;
  Sync: { O: number; C: number }; //和Recorder.Sync一致，只不过这个是非全局的，仅用来简化代码逻辑，无实际作用
  _O?: number;
  _O_?: number;
  _S: number;
  _SO?: number;
  ...
  static IsOpen(): boolean {
    let stream: any = Recorder.Stream;
    if (stream) {
      let tracks = (stream.getTracks && stream.getTracks()) || stream.audioTracks || [];
      let track = tracks[0];
      if (track) {
  ...
  static Destroy() {
    ...
    for (const k in DestroyList) {
      DestroyList[k]();
    }
  }

  //登记一个需要销毁全局资源的处理方法
  static BindDestroy(key: string, call: Function) {
  ...
  static GetContext(): AudioContext | null {
    ...
    if (!AC) {
      AC = (window as any).webkitAudioContext;
    }
    if (!AC) {
      return null;
    }
  ...
  */
  static SampleData(
    ...
    prevChunkInfo?: { index?: number; offset?: number; frameNext?: Int16Array[]; frameSize?: number },
    ...
    let frameNext: any = prevChunkInfo.frameNext || [];
    option || (option = {});
    ...
    let res: any = new Int16Array(size);
    let idx = 0;
    //添加上一次不够一帧的剩余数据
  ...
  */
  static PowerDBFS(maxSample: number): number {
  ...
  open(True: Function, False?: Function) {
    let streamStore = this._streamStore();
    True = True || NOOP;
    let failCall = (errMsg: string, isUserNotAllow?: boolean) => {
      isUserNotAllow = !!isUserNotAllow;
      console.log("录音open失败：" + errMsg + ",isUserNotAllow:" + isUserNotAllow, 1);
    ...
    //***********打开麦克风得到全局的音频流************
    const codeFail = (code: string, msg: string) => {
      try {
        //跨域的优先检测一下
        if ((window.top as any).a) {
        } else {
    ...
    let f1 = (stream: MediaStream | any) => {
      ...
      setTimeout(() => {
        stream._call = {};
        let oldStream: any = Recorder.Stream;
        if (oldStream) {
    ...
    let trackSet: MediaTrackConstraints = {
    ...
    if (pro && pro.then) {
      pro.then(f1).catch(f2); //fix 关键字，保证catch压缩时保持字符串形式
    }
  }
  ...
  close(call?: Function) {
    ...
    if (this._O_ != Lock.O) {
      ...
      console.log("close被忽略（因为同时open了多个rec，只有最后一个会真正close）", 3);
      call();
      return;
    }
    Lock.C++; //获得控制权
  ...
  mock(pcmData: Int16Array, pcmSampleRate: number) {
    this._stop(); //清理掉已有的资源
  ...
  envCheck(envInfo: { envName: string; canProcess: boolean }): string {
  ...
  envStart(mockEnvInfo: any, sampleRate: number) {
    //平台环境相关的start调用
    let set = this.set;
  ...
  envIn(pcm: Int16Array, sum: number) {
    // console.log("envInenvInenvInenvIn");
    ...
    if (lost > tsIn / 3 && ((tsInPrev && tsIn > 1000) || envInFixTs.length >= 6)) {
      ...
      if (addTime > pcmTime / 5) {
        ...
        console.log("[" + now + "]" + (fixOpen ? "" : "未") + "补偿" + addTime + "ms", 3);
        if (this.envInFix) {
    ...
    if (engineCtx) {
      //转换成set的采样率
      let chunkInfo = Recorder.SampleData(buffers, bufferSampleRate, set[sampleRateTxt], engineCtx.chunkInfo);
    ...
    let asyncEnd = () => {
      ...
      let hasClear: any = buffers[0] == null;
    ...
    let asyncBegin: any = 0;
    let procTxt = "rec.set.onProcess";
    try {
    ...
    if (slowT > 10 && this.envInFirst && this.envInFirst - now > 1000) {
      //1秒后开始onProcess性能监测
  ...
  start() {
    ...
    // console.log("开始录音，麦克风isOpen：", isOpen);
  ...
  */
  stop(True: Function, False?: Function, autoClose?: boolean) {
    let set = this.set;
    let t1: number = 0;
    if (t1) {
      //防止ts报错
    ...
    // let envInLen: any = envInMS && this.buffers?.length; //可能未start

    // let log_params = "stop 和 start 时差";
    // if (envInMS) {
    //   log_params += envInMS + "ms 补偿" + this.envInFix + "ms" + " envIn:" + envInLen + " fps:";
    //   log_params += ((envInLen / envInMS) * 1000).toFixed(1);
    // } else {
    //   log_params += "-";
    // }
    // console.log(log_params);
    ...
    let ok = (blob: Blob, duration: number) => {
      // console.log("结束录音 编码花" + (Date.now() - t1) + "ms 音频时长" + duration + "ms 文件大小" + blob.size + "b");
      if (set.takeoffEncodeChunk) {
        //接管了输出，此时blob长度为0
        console.log("启用takeoffEncodeChunk后stop返回的blob长度为0不提供音频数据", 3);
    ...
    // console.log(this, set.type, set, this.type, !this[set.type]);
    if (!this[set.type]) {
      console.log(1111);
      err("未加载" + set.type + "编码器");
      return;
    }
    // console.log("222222222222222222", set);
    //环境配置检查，此处仅针对mock调用，因为open已经检查过了
    if (this.isMock) {
      let checkMsg = this.envCheck(this.mockEnvInfo || { envName: "mock", canProcess: false }); //没有提供环境信息的mock时没有onProcess回调
      if (checkMsg) {
        err("录音错误：" + checkMsg);
        return;
      }
    ...
    if (this[set.type + "_complete"] && engineCtx) {
      let duration = Math.round((engineCtx.pcmSize / set[sampleRateTxt]) * 1000); //采用后的数据长度和buffers的长度可能微小的不一致，是采样率连续转换的精度问题
    ...
    //标准UI线程转码，调整采样率
    t1 = Date.now();
    let chunk = Recorder.SampleData(this.buffers, this[srcSampleRateTxt], set[sampleRateTxt]);
  ...
  pcm(res?: any, True?: any, False?: any) {
    if (False) {
      //避免ts报错
    }
    let set = this.set;
    let size = res.length;
    let bitRate = set.bitRate == 8 ? 8 : 16;
